...
export default function FiiDetailsModal({
  ...
  // Verifica se está na carteira
  const walletItem = MOCK_WALLET.find((w) => w.ticker === fii.ticker);
  ...
  const handleSave = () => {
    ...
    // Atualiza o Mock (Simulação de Banco de Dados)
    const existingIdx = MOCK_WALLET.findIndex(w => w.ticker === fii.ticker);
    if (existingIdx >= 0) {
        MOCK_WALLET[existingIdx] = { ...MOCK_WALLET[existingIdx], qty: newQty, avg_price: newPrice };
    } else {
        MOCK_WALLET.push({ ticker: fii.ticker, qty: newQty, avg_price: newPrice });
    }
...
{/* === COLUNA ESQUERDA (SIDEBAR / HEADER NO MOBILE) === */}
        <div className="w-full md:w-1/3 bg-[#1e293b]/50 md:border-r border-slate-700 flex flex-col shrink-0 transition-all">
          ...
          <div className="p-5 md:p-6 pb-2 md:pb-6 flex flex-col h-full">
            ...
            {/* 1. HEADER DO ATIVO */}
            {/* ADICIONADO: 'mb-6' para forçar distância dos botões no mobile */}
            <div className="flex flex-row md:flex-col justify-between items-start md:items-stretch gap-4 mb-6 md:mb-0">
              <div>
                {/* Badges */}
                {/* AUMENTADO: 'mb-3' para dar ar entre badges e ticker */}
                ...
                {/* Ticker e Nome */}
                {/* AUMENTADO: 'mb-2' para separar o nome do container */}
            ...
            {/* --- ÁREA DINÂMICA: FORMULÁRIO OU DETALHES --- */}
            {isEditing ? (
                // === MODO EDIÇÃO ===
                ...
                <>
                    ...
                    {/* 5. BOTÕES DE AÇÃO */}
                    {/* ADICIONADO: 'pt-4' no mobile para garantir separação se mt-auto falhar */}
                    <div className="mt-auto px-5 pb-5 md:px-6 md:pb-6 pt-4 md:pt-6 md:border-t border-slate-700 space-y-4 md:space-y-3">
                        <button 
                            onClick={handleStartEdit}
                            className="w-full bg-blue-600 hover:bg-blue-500 text-white py-2.5 md:py-3 rounded-xl text-xs md:text-sm font-bold transition-all shadow-lg shadow-blue-500/20 flex items-center justify-center gap-2"
                        >
        ...
        {/* === COLUNA DIREITA (CONTEÚDO) - Intocada === */}
