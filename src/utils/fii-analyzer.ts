...
export function analyzeFII(fii: FiiData): AnalysisResult {
  ...
  // 5. Estabilidade (Variação de Dividendos)
  ...
  if (fii.dividends) {
      ...
      if (Array.isArray(divs) && divs.length >= 6) {
          ...
          if (cv > 25) {
              score -= 15;
              warnings.push({ pillar: "Estabilidade", msg: "Alta oscilação nos dividendos.", severity: "medium" as const });
              cvStatus = "Instável";
          } else if (cv > 15) {
              score -= 5;
              cvStatus = "Alerta";
  ...
  return { score, warnings, veredict, cvStatus, dyStatus, pvpStatus };
}
